...
tools = st.sidebar.multiselect("Select tools to display", options=df["tool"].unique(), default=df["tool"].unique())
min_comments = st.sidebar.slider("Minimum comments per month (filters out noisy months)", min_value=1, max_value=int(df["comment_count"].max()), value=5)
...
st.markdown(f"**Reading the chart:** dot size shows how many comments back each point. Bigger dots are more trustworthy. At the current filter, **{most_negative_tool}** trends most negative and **{most_positive_tool}** trends most positive.")

fig = px.scatter(filtered, x="month", y="avg_sentiment", color="tool", size="comment_count", size_max=40, hover_data=["comment_count"], title="Monthly Sentiment by Tool (dot size = number of comments)")
...
for tool in tools:
    ...
    st.markdown(f"**{tool}**")
    c1, c2 = st.columns(2)
    top_positive = tool_comments[tool_comments["sentiment"] == "positive"].sort_values("sentiment_score", ascending=False).head(2)
    top_negative = tool_comments[tool_comments["sentiment"] == "negative"].sort_values("sentiment_score", ascending=False).head(2)
    with c1:
